...
def main():
    ...
    parser.add_argument('--reload_item_embeddings', type=bool, default=False)
    ...
    path_ckpt = path_output / f'best_stage_1_model_{timestamp}.bin'
    ...
    if args.reload_item_embeddings:
        print('Load best item embeddings in stage 1.')
        item_embeddings = encode_all_items(model.longformer, tokenizer, tokenized_items, args)
        model.init_item_embedding(item_embeddings)

    path_ckpt = path_output / f'best_stage_2_model_{timestamp}.bin'
    torch.save(model.state_dict(), path_ckpt)
    patient = 3
    ...
    # save the metrics
    path_metrics = path_output / f'finetuned_metrics_{timestamp}.json'    
    json.dump(test_metrics, open(path_metrics, 'w'), indent=4)
